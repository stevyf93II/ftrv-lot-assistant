...
// NOTE (2026-07-05): Front Bath / Rear Bath deliberately have NO alias here.
// They exist ONLY as scraper-emitted features[] strings sourced from FTRV's
// own faceted filters (features-prop-4). The old aliases to u.bath_loc let
// wrong floorplan-code guesses leak in ("front bath" 5Ws had zero overlap
// with FTRV's own filter).
const FEATURE_ALIAS = {
...
function applyFilterSpec(spec, startList) {
  ...
  if (spec.bath_loc) {
    // bath_loc guessing retired 2026-07-05 — FTRV site feature tags are truth.
    // Legacy/cached specs that still emit bath_loc map to the truth features.
