...
//
// SPEC-ONLY DOCTRINE (2026-07-11): the AI is a query translator, nothing more.
// No narrative field — the tool is sales-pro-only, never customer-facing, and
// the pro doesn't need to be sold to. The AI's judgment lives in the MAPPING
// (edge-case phrasing → correct schema fields); everything the user sees is
// computed deterministically by JS (counts, facts line, near-miss tiers).
//
...
function aliasUnit(u) {
  ...
  u.monthly = u.monthly ?? u.monthly_from;
...
function applyFilterSpec(spec, startList) {
  ...
  if (spec.maxMonthly != null) list = list.filter(u => (u.monthly || 0) > 0 && u.monthly <= spec.maxMonthly);
  if (spec.minMonthly != null) list = list.filter(u => (u.monthly || 0) >= spec.minMonthly);
...
// Near-miss fallback (Steve 2026-07-11): whenever strict matches come back
// UNDER 4, find the SINGLE constraint whose removal adds the most units.
// Returns one of:
//   { units: strict, fallback: null }
//       — 4+ strict matches, or no single drop adds anything
//   { units: strict, fallback: { dropped, dropped_value, strict_count: 1-3, restored_count, extra_units } }
//       — 1-3 strict matches; extra_units = relaxed matches NOT in strict
//   { units: relaxedList, fallback: { dropped, dropped_value, strict_count: 0, restored_count } }
//       — 0 strict matches (legacy zero-case contract: units IS the relaxed list)
function applyFilterSpecWithFallback(spec, startList) {
  const strict = applyFilterSpec(spec, startList);
  if (strict.length >= 4) return { units: strict, fallback: null };
  ...
  const tryDrop = (label, dropFn) => {
    ...
    if (matches.length > strict.length) candidates.push({ dropped: label, count: matches.length, units: matches, dropped_value: spec[label.split(' ')[0]] });
  };

  // Numeric cap relaxations (most common cause of thin matches)
  if (spec.maxPrice != null) tryDrop('maxPrice', s => ({ ...s, maxPrice: undefined }));
  if (spec.maxMonthly != null) tryDrop('maxMonthly', s => ({ ...s, maxMonthly: undefined }));
  ...
  if (!candidates.length) return { units: strict, fallback: null };

  // Pick the SINGLE drop yielding the most matches.
  candidates.sort((a, b) => b.count - a.count);
  const best = candidates[0];

  if (strict.length === 0) {
    // Legacy zero-case contract: units IS the relaxed list.
    return {
      units: best.units,
      fallback: {
  ...
  // 1-3 strict matches: keep strict as the primary result, hand back the
  // near-miss expansion separately so the UI can show "N exact · M more".
  const strictSet = new Set(strict.map(u => u.stock));
  const extra = best.units.filter(u => !strictSet.has(u.stock));
  return {
    units: strict,
    fallback: {
      dropped: best.dropped,
...
function describeFilterSpec(spec, sortBy, sortDir, limit) {
  ...
  if (spec.maxMonthly != null) parts.push('≤$' + spec.maxMonthly.toLocaleString() + '/mo');
  if (spec.minMonthly != null) parts.push('≥$' + spec.minMonthly.toLocaleString() + '/mo');
...
// Returns { spec, sortBy, sortDir, limit, noMatch, note } — no narrative
// (SPEC-ONLY DOCTRINE 2026-07-11).
